...
export default function EditarRespuestaPage() {
  const { isAuthenticated, user } = useSimpleAuth()
  const router = useRouter()
  const params = useParams()
  const id = params?.id as string

  const [respuesta, setRespuesta] = useState<RespuestaAutomatica | null>(null)
  ...
  const loadRespuesta = async () => {
    ...
    try {
      ...
      if (data.respuesta) {
        ...
        setNombre(data.respuesta.nombre)
        setAsunto(data.respuesta.asunto)
        setMensaje(data.respuesta.mensaje)
        setTriggerTipo(data.respuesta.trigger_tipo)
        setActiva(data.respuesta.activa)
      } else {
      ...
      alert('Error al cargar respuesta')
    }
    setLoading(false)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert('Respuesta automatica actualizada correctamente')
        router.push('/crm/cliente/respuestas-automaticas')
      } else {
        const data = await res.json()
      ...
      alert('Error al actualizar respuesta automatica')
    }
    setSaving(false)
  }
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 text-white shadow-lg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <h1 className="text-3xl font-bold">Editar Respuesta Automatica</h1>
          </div>
        </div>
        ...
        {respuesta && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <p className="text-sm text-blue-800">
              <strong>Creado:</strong> {new Date(respuesta.creado_en).toLocaleString('es-CL')} |{' '}
              <strong>Ultima actualizacion:</strong> {new Date(respuesta.actualizado_en).toLocaleString('es-CL')}
            </p>
          </div>
        )}
